...
def obter_tipos(search: str, db_conn) -> list[str]:
    try:
        with db_conn.cursor() as db_cursor:
            db_cursor.execute(
                "SELECT tipo FROM tipo_insumo WHERE tipo ILIKE %s LIMIT 10;", (f"{search}%",)
            )
...
def filter_tipos(combobox: ttk.Combobox, db_conn):
...
def registrar_frame_cadastro_insumo(notebook: ttk.Notebook, db_conn) -> None:
    """
    ...
    header = ttk.Label(container, text="Cadastro de Insumos", font="-size 14 -weight bold")
    ...
    aquisicao_label = ttk.Label(container, text="Data de Aquisição:", font="-weight bold")
    ...
    def salvar_insumo():
        ...
        data = aquisicao_entry.get_date()  # Retorna datetime.date
        ...
        if not nf or not cnpj or not tipo or not custo_str or not quant_str:
            messagebox.showwarning("Dados Incompletos", "Por favor, preencha todos os campos do formulário.")
            return

        if len(nf) != 44:
            messagebox.showwarning("Nota Fiscal Inválida", "A chave de acesso da Nota Fiscal deve conter exatamente 44 dígitos.")
            return

        if len(cnpj) != 14:
            messagebox.showwarning("CNPJ Inválido", "O CNPJ do fornecedor deve conter exatamente 14 dígitos.")
            return
        ...
        try:
            ...
            messagebox.showwarning("Custo Inválido", "O custo deve ser um número decimal válido e não-negativo (ex: 1250.75).")
            return
        ...
        try:
            ...
            messagebox.showwarning("Quantidade Inválida", "A quantidade deve ser um número inteiro positivo maior que zero.")
            return
        ...
        try:
            with db_conn.cursor() as cur:
                ...
                if not cur.fetchone():
                    ...
                    cur.execute("SELECT nome_fantasia FROM pessoa_juridica WHERE cnpj = %s;", (cnpj,))
                    pj_row = cur.fetchone()
                    if pj_row:
            ...
            messagebox.showerror(
                "Erro Inesperado",
                f"Ocorreu um erro inesperado ao salvar:\n\n{str(e)}"
            )
    ...
    btn_salvar = ttk.Button(container, text="  Salvar Insumo  ", command=salvar_insumo, style="Accent.TButton")
...
def registrar_frame_consulta_ordens(notebook: ttk.Notebook, db_conn) -> None:
    """
    ...
    btn_buscar = ttk.Button(search_frame, text="Buscar", command=lambda: realizar_busca(), style="Accent.TButton")
    ...
    tree.column("id_ordem", width=90, anchor="center")
    tree.column("gerente", width=220, anchor="w")
    tree.column("inicio", width=160, anchor="center")
    tree.column("fim", width=160, anchor="center")
    tree.column("gasto_energia", width=130, anchor="e")
    tree.column("gasto_agua", width=130, anchor="e")
        ...
        try:
            with db_conn.cursor() as cur:
                ...
                for i, row in enumerate(rows):
                    gasto_energia = f"{float(row[4]):.3f}"
                    gasto_agua = f"{float(row[5]):.3f}"
                    # Alterna o tag para criar efeito zebra (linhas pares e ímpares)
                    tag = "par" if i % 2 == 0 else "impar"
                    tree.insert("", "end", values=(row[0], row[1], row[2], row[3], gasto_energia, gasto_agua), tags=(tag,))
...
def monta_ui(root: Tk, db_conn) -> None:
    ...
    status_lbl = ttk.Label(top_bar, text="• Gestão Operacional", font="-size 10", foreground="#2E7D32")
    ...
    def toggle_theme():
        if switch_var.get():
            ...
            status_lbl.configure(foreground="#81C784")  # Tom de verde claro para fundo escuro
            # Zebra stripes adaptadas para o modo escuro
            ordens_tree.tag_configure("par",   background="#2d2d2d")
            ordens_tree.tag_configure("impar", background="#383838")
        else:
            style.theme_use("forest-light")
            status_lbl.configure(foreground="#2E7D32")  # Tom de verde escuro para fundo claro
            # Zebra stripes adaptadas para o modo claro
            ordens_tree.tag_configure("par",   background="#f7f9f7")
            ordens_tree.tag_configure("impar", background="#ffffff")
    ...
    theme_switch = ttk.Checkbutton(top_bar, text="Modo Escuro", style="Switch", variable=switch_var, command=toggle_theme)
...
if __name__ == "__main__":
    db_conn = None
    try:
        db_conn = psy.connect(host="localhost", port=5433, dbname=DBNAME, user=DBUSER, password=DBPASS)
    except Exception as e:
